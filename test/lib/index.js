...
if (require.main === module) {
    commander_1.program
        ...
        try {
            console.log('ICA address: ' + JSON.stringify(await connection.client.queryContractSmart(claimerAddress, { interchain_account: {} })));
        }
        catch {
            console.log('No ica account');
        }
        console.log('Stage: ' + JSON.stringify(await connection.client.queryContractSmart(claimerAddress, { stage: {} })));
        try {
            console.log('TransferAmount: ' + await connection.client.queryContractSmart(claimerAddress, { transfer_amount: {} }));
        }
        catch {
            console.log('No transfer amount');
        }
    ...
    commander_1.program
        ...
        const connection = await (0, connector_1.Connect)(wallet, endpoint);
        await (0, setup_1.SetupContracts)(connection, connectionId, channelId, ibcNeutronDenom);
        console.log('🥳 Done');
    });
    commander_1.program
        .name('step_1')
